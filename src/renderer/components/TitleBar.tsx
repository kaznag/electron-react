...
const TitleBar: React.FC<TitleBarProps> = props => {
  ...
  return (
    <div className="title-bar">
      ...
      <div className={(isMaximized ? 'resize-button' : 'maximize-button') + (!isFocused ? ' blur' : '')} onClick={props.onMaximizeResizeButtonClick} data-test-id='maximize-button'></div>
      <div className={'minimize-button' + (!isFocused ? ' blur' : '')} onClick={props.onMinimizeButtonClick} data-test-id='minimize-button'></div>
      <div className={'window-title' + (!isFocused ? ' blur' : '')} data-test-id='window-title'>{props.windowTitle}</div>
    </div >
  );
};
